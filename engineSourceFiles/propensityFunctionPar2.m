function a = propensityFunctionPar2(X,Cs_tPA, Cs_PLG, Cs_PLS,epsilon)
...
% Ads, PLG Dis
    ...
    A = zeros(1, N_CS); PLS_array = X(1 + 2*N_CS:3*N_CS);
    b = zeros(1, N_CS); PLG_array = X(1 + 1*N_CS:2*N_CS);
    ...
    for it_G = 1:N_CS
        ...
        % PLS desorption/ adsorption
        A(it_G) = k_r_PLS*PLS_array(it_G);
        b(it_G) = k_a_PLS*(Nfs_PLS/epsilon)*N_free(it_G);
        ...
        % Cat reaction
        d(it_G) = k_cat_new*PLS_array(it_G);
        
        % L_PLS dis 
        e(it_G) = k_r_PLS*L_PLS_array(it_G);
        
        % tPA adsorption/desorption
        f(it_G) = k_a_tPA*(Nfs_tPA(it_G)/epsilon)*N_free(it_G);
        g(it_G) = k_r_tPA*X(it_G);
        
        % PLG adsorption/desorption; Both are zero here
    end
    
    %clear PLS_array PLG_array L_PLS_array
    a = zeros(1, length(X));
    
    a(1:N_CS) = A; % PLS desorption
    a(N_CS+1:2*N_CS) = b; % PLS adsorption
    a(2*N_CS+1:3*N_CS) = c; % M-M reaction
    a(3*N_CS+1:4*N_CS) = d; % Cat reaction
    a(4*N_CS+1:5*N_CS) = e; % L_PLS dis
    a(5*N_CS+1:6*N_CS) = f; % tPA adsorption
    a(6*N_CS+1:7*N_CS) = g; % tPA desorption
