function a = propensityFunction(X,Cs_tPA, Cs_PLG, Cs_PLS,epsilon)
...
% Ads, PLG Dis
    ...
    k_2_new = k_2;
        ...
        for it_G = 1:N_CS
            % M-M reaction
            c(it_G) = k_2_new*PLG_array(it_G)*X(it_G)/(K_Mn + PLG_array(it_G));
            if (check_new == 1)
    ...
    a(N_CS+1:2*N_CS) = k_a_PLS*(Nfs_PLS)*N_free; % PLS adsorption
    ...
    a(5*N_CS+1:6*N_CS) = k_a_tPA*(Nfs_tPA(it_G)).*N_free; % tPA adsorption
